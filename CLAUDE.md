...
**v2 rebuild** — `data/wc2022_players_master.csv`: 217 rows × 193 columns (146 pre-P3 + 21 P3 + 23 P6 event-derived + 2 identity columns incl. `position_v2` + a `position_detail` fine-position label), FIFA World Cup 2022 (StatsBomb competition 43 / season 106). Built by `build_master_dataset.py` from FBref CSVs + StatsBomb events; 217 eligible players after Min≥270 filter. **P3+P6 complete** — 44 locked event-derived features: P3 (pressures, recoveries, touches by zone, GK metrics) at `7ccb424`, P6 (passing, defending/duels, shots/xG/npxG, box/final-third touches, penalties) + `position_v2` (6 groups from most-played StatsBomb lineup position: GK=28, CB=59, FB/WB=36, MF=55, Wide=21, ST=18) on branch `statsbomb-parser`. `position_detail` (ADR-013) adds fine labels (DM/CM/AM/LM/RM, LW/RW, LB/RB/LWB/RWB) for display only. Raw StatsBomb Open Data lives in `data/statsbomb/` (gitignored; reproducible via `scripts/download_statsbomb.py`).

**v2 engine (P7)** — `v2_model_engine.py` clusters the master by `position_v2` group (k=2/3/3/5/3/3) and labels each cluster against the **20 σ-offset archetypes** (traits as σ-above-group-mean, converted to raw units via the player-level scaler). Persists to `models_v2/` (gitignored), invalidated by SHA256 of the CSV. `python v2_model_engine.py --persist` fits+saves; `--evaluate` fits+logs per-group silhouette/DB + label distribution.
...
### 2026-08-14 — Fine-grained position labels (display-only, ADR-013) (branch `worktree-p8-bootstrap-stability`)
- **position_detail added:** the StatsBomb lineup data already carried fine side/role positions ("Left Back", "Right Wing", …), but `parse_lineups` discarded them. Added `POSITION_FINE_MAP` (raw position → fine label) + a `position_detail` output in `parse_lineups` and `merge_position_v2`, then regenerated the master (now **217 × 193 cols**). Fine labels: MF→DM/CM/AM/LM/RM, Wide→LW/RW, FB/WB→LB/RB/LWB/RWB; CB/ST/GK stay coarse. **Clustering is unchanged** (still on coarse `position_v2`). Distribution: CB 59 · DM 32 · GK 28 · LB 19 · ST 18 · CM 16 · RB 13 · LW 12 · RW 9 · RM 4 · AM 3 · LWB 2 · RWB 2.
- **UI:** the table "Position" column, sidebar "Position" filter, and display now use `position_detail` (fine); the archetype browser + clustering keep the coarse group. `filter_v2_dataframe` renamed `groups` → `positions`.
- **Tests:** parser fine-map test + `position_detail` assertions in `parse_lineups`/`merge_position_v2` tests; v2_features filter tests updated. Full suite green + ruff clean.
- **Docs updated:** `DECISIONS.md` ADR-013, `ARCHITECTURE.md` §11, this entry.
