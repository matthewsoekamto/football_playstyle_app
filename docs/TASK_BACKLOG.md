...
Authority: subordinate to `PROJECT_CONSTITUTION.md`. Every item below was identified by direct inspection of the current repository — none are generic placeholder tasks. IDs are stable references used elsewhere in `/docs` (e.g. `ML_GUIDELINES.md` refers to `ML-01`).
...
- **Expected outcome:** The app degrades safely instead of crashing for end users. See `STYLE_GUIDE.md §9`, `STREAMLIT_GUIDELINES.md §12`, `SECURITY_GUIDE.md §2`.
...
- **Acceptance criteria:** At minimum, a test proving `group_players` produces identical `playstyle_cluster` labels across two runs on the same input (the single highest-value test per `TESTING_GUIDE.md §3`).
...
- **Expected outcome:** No misleading always-tied percentile columns sitting in the DataFrame. See `DECISIONS.md` ADR-007, `ML_GUIDELINES.md §6`.
...
- **Expected outcome:** No dead/misleading parameter in a public function signature. See `STYLE_GUIDE.md §12`.
...
- **Expected outcome:** Reduced supply-chain/reproducibility risk. See `SECURITY_GUIDE.md §7`.
...
- **Expected outcome:** Auditable, versioned model artifacts instead of implicit "the model is whatever the code + CSV currently produce." See `ML_GUIDELINES.md §11`, `PERFORMANCE_GUIDE.md §6`.

### ML-04: Add clustering evaluation metrics
...
- **Expected outcome:** Objective, reviewable evidence of cluster quality — currently zero such metrics exist anywhere in the project. See `ML_GUIDELINES.md §9`.
...
- **Acceptance criteria:** Every public function in `data_loader.py`, `model_engine.py`, `features.py` has at least one test; every `build_*` in `charts.py` has a smoke test — per `TESTING_GUIDE.md §10`.
- **Expected outcome:** Meets the Definition of Production Ready testing bar (`PROJECT_CONSTITUTION.md §16`).
...
- **Expected outcome:** Meets the Definition of Production Ready CI bar (`PROJECT_CONSTITUTION.md §16`).
...
- **Expected outcome:** Cleaner script; no functional change (verify the removal truly has zero effect before merging, per `AI_DEVELOPER_RULEBOOK.md`'s "never remove functionality unless requested/verified safe" spirit — in this case the line's output is provably unused).
...
- **Acceptance criteria:** `comp`, `squad`, `primary_position`, `Playstyle` become `pd.Categorical` — only pursue once row counts materially grow (`PERFORMANCE_GUIDE.md §3`).
...
- **Acceptance criteria:** A season selector replaces the hardcoded filename; cache keys correctly incorporate season; clustering remains season-scoped per `ML_GUIDELINES.md §14`.
...
- **Acceptance criteria:** Cache key incorporates file mtime or content hash so replacing the CSV without a process restart correctly invalidates cached data. See `PERFORMANCE_GUIDE.md §4`.
