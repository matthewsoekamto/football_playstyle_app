...
### P8: v2 clustering evaluation (silhouette, DB, stability/bootstrap) — ✅ RESOLVED
- **Priority:** Medium | **Effort:** ~1 day
- **Status:** RESOLVED (2026-08-12, branch `worktree-p8-bootstrap-stability`). `evaluate_bootstrap_stability` added to `v2_model_engine.py --evaluate`: per `position_v2` group, B=100 same-n bootstrap resamples (seeded from `RANDOM_STATE`, refit stream offset by B), each refits the exact production preprocessing (`fillna(0) → StandardScaler → KMeans(k, seeded, n_init=10)`) and predicts labels for **all original players**; scores `adjusted_rand_score` vs the deployed partition → **mean ± std ARI** (bootstrap stability), mean ± std refit silhouette/DB (refit-variance), and a **degenerate fraction** (full-n prediction collapsed to ≤1-player cluster). Real-data snapshot: GK ARI 0.633±0.287 (degen 0.01) · CB 0.344±0.183 (0.06) · FB/WB 0.203±0.129 (0.17) · MF 0.354±0.142 (0.12) · Wide 0.304±0.152 (0.26) · ST 0.454±0.191 (0.39). As predicted, low-silhouette groups (ST/Wide) are also the least stable — ST's 4-way split is **provisional** (candidate for k-reduction in a future phase), reported as a limitation, not a blocker. Methodology in `DECISIONS.md` ADR-010; `ARCHITECTURE.md` §11.
