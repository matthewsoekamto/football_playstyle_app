...
- **Data ingestion & cleaning** (`data_loader.load_and_clean_data`): loads `players_data_light-2025_2026.csv` (2,839 rows × 53 raw FBref-style columns), normalizes column names, filters to players with ≥270 minutes played (2,183 remain), derives `primary_position` from the first token of the (possibly multi-valued, e.g. `"MF,FW"`) `Pos` column, and computes per-90 rate stats for six outfield stats (gls, ast, soT, CrdY, CrdR, touches) and three goalkeeper stats (GA, saves, CS).
- **Clustering** (`model_engine.group_players`): two independent K-Means models — 8 clusters for outfield players on 6 features, 2 clusters for goalkeepers on 4 features — each with `StandardScaler` and `random_state=42` for reproducibility. Cluster centroids are matched to one of 10 hand-authored archetype definitions (8 outfield, 2 GK) to produce human-readable playstyle labels.
